...
def load_txt_file(file_path, debug=True):
    ...
    if debug:
        assert is_path_exists(file_path), (
            "text file is not existing at path: %s!" % file_path
        )
...
def load_list_from_folder(
    ...
    if debug:
        assert isfolder(folder_path), (
            "input folder path is not correct: %s" % folder_path
        )
    if not is_path_exists(folder_path):
        print("the input folder does not exist\n")
        return [], 0
    if debug:
        assert islogical(recursive), (
            "recursive should be a logical variable: {}".format(recursive)
        )
        assert depth is None or (isinteger(depth) and depth >= 1), (
            "input depth is not correct {}".format(depth)
        )
        assert (
            ext_filter is None
