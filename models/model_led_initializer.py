...
class LEDInitializer(nn.Module):
    def __init__(
        self, t_h: int = 8, d_h: int = 6, t_f: int = 40, d_f: int = 2, k_pred: int = 20
    ):
        """
        Parameters
        ----
        ...
        self.var_decoder = MLP(
            256 * 2 + 32, self.output_dim, hid_feat=(1024, 1024), activation=nn.ReLU()
        )
        self.mean_decoder = MLP(
            256 * 2, t_f * d_f, hid_feat=(256, 128), activation=nn.ReLU()
        )
    ...
    def forward(self, x, mask=None):
        ...
        guess_mean = (
            self.mean_decoder(mean_total).contiguous().view(-1, self.fut_len, 2)
        )
        ...
        guess_var = self.var_decoder(var_total).reshape(
            x.size(0), self.n, self.fut_len, 2
        )
